"""Claude Provider — Anthropic Claude API wrapper with retry and output parsing.

Provides a production-ready integration with the Anthropic Claude API,
implementing the ILLMInterface contract from decision_ports.py.

Features:
- Structured prompt dispatch via PromptBuilder.
- System prompt prefix caching (Anthropic cache_control).
- Exponential backoff retry via RetryManager.
- 4-stage output parsing via OutputParser.
- JSON schema validation via JSONValidator.
- Zero hardcoded API keys (reads from env/settings).

Spec: llm_strategy.md §2 (Tier 1: Fast Single-Pass Router).
      deployment.md §3 (Secrets & API Key Governance).
"""

from __future__ import annotations

import logging
import time
from typing import Any, Dict, List, Optional

from router.application.prompts.prompt_builder import BuiltPrompt
from router.infrastructure.llm.output_parser import OutputParser, ParseResult
from router.infrastructure.llm.retry_manager import MaxRetriesExceededError, RetryManager

logger = logging.getLogger(__name__)

# Model identifiers (llm_strategy.md §2)
_DEFAULT_CLAUDE_MODEL = "claude-3-5-haiku-20241022"
_CLAUDE_DEEP_MODEL = "claude-3-5-sonnet-20241022"

# API call timeout (seconds)
_DEFAULT_TIMEOUT_SECS = 10.0


class ClaudeProviderError(Exception):
    """Raised when the Claude API call fails after all retries."""


class ClaudeProvider:
    """Anthropic Claude API provider with retry and output parsing.

    Wraps the Anthropic Python SDK with production resilience features:
    - Retry logic with exponential backoff (max 3 attempts).
    - System prompt prefix caching for cost reduction.
    - Full multi-stage JSON repair pipeline.

    Args:
        api_key: Anthropic API key. Reads from ANTHROPIC_API_KEY env if None.
        model: Claude model identifier (default: claude-3-5-haiku-20241022).
        retry_manager: Optional custom RetryManager (uses default if None).
        output_parser: Optional custom OutputParser (uses default if None).
        timeout_secs: API call timeout in seconds.
    """

    def __init__(
        self,
        api_key: Optional[str] = None,
        model: str = _DEFAULT_CLAUDE_MODEL,
        retry_manager: Optional[RetryManager] = None,
        output_parser: Optional[OutputParser] = None,
        timeout_secs: float = _DEFAULT_TIMEOUT_SECS,
    ) -> None:
        """Initialize ClaudeProvider.

        Args:
            api_key: Anthropic API key.
            model: Model identifier string.
            retry_manager: Retry manager instance.
            output_parser: Output parser instance.
            timeout_secs: Request timeout.
        """
        self._api_key = api_key or self._read_api_key()
        self._model = model
        self._retry_manager = retry_manager or RetryManager()
        self._output_parser = output_parser or OutputParser()
        self._timeout_secs = timeout_secs
        self._client = self._init_client()
        logger.info(
            "ClaudeProvider initialized",
            extra={"model": model, "client_available": self._client is not None},
        )

    def complete(self, prompt: BuiltPrompt) -> Dict[str, Any]:
        """Send a prompt to Claude and return the parsed JSON response.

        Args:
            prompt: BuiltPrompt assembled by PromptBuilder.

        Returns:
            Parsed and validated JSON dict with keys: action, reason, confidence, evidence.

        Raises:
            ClaudeProviderError: If the API call fails after all retries.
        """
        if self._client is None:
            logger.warning("ClaudeProvider: No API client; returning mock response")
            return self._mock_response()

        start_time = time.perf_counter()

        try:
            result = self._retry_manager.execute(
                lambda: self._call_api(prompt)
            )
            raw_response = result.value
        except MaxRetriesExceededError as exc:
            raise ClaudeProviderError(
                f"Claude API failed after {exc.attempts} attempts: {exc.last_error}"
            ) from exc

        latency_ms = (time.perf_counter() - start_time) * 1000.0

        # Parse the response
        parse_result: ParseResult = self._output_parser.parse(raw_response)

        logger.info(
            "ClaudeProvider: response received",
            extra={
                "model": self._model,
                "latency_ms": round(latency_ms, 2),
                "repair_applied": parse_result.repair_applied,
                "repair_stage": parse_result.repair_stage,
                "action": parse_result.action,
                "confidence": parse_result.confidence,
                "cache_hit": prompt.is_cache_hit,
            },
        )

        return parse_result.parsed

    def _call_api(self, prompt: BuiltPrompt) -> str:
        """Execute a single Claude API call.

        Args:
            prompt: BuiltPrompt with system and user content.

        Returns:
            Raw text response string from Claude.

        Raises:
            Exception: On API failure (will be caught by RetryManager).
        """
        messages = [{"role": "user", "content": prompt.user_prompt}]
        system_content = self._build_system_content(prompt)

        response = self._client.messages.create(  # type: ignore[union-attr]
            model=self._model,
            max_tokens=prompt.api_params.get("max_tokens", 150),
            temperature=prompt.api_params.get("temperature", 0.0),
            system=system_content,
            messages=messages,
        )

        return response.content[0].text

    def _build_system_content(self, prompt: BuiltPrompt) -> Any:
        """Build system content with optional cache_control for prefix caching.

        Anthropic prefix caching: Mark the system prompt as 'ephemeral' to
        enable server-side caching (reduces billing tokens by ~50%).

        Args:
            prompt: BuiltPrompt containing system text.

        Returns:
            System content string or list with cache_control.
        """
        # Enable Anthropic prompt caching for eligible system prompts
        return [
            {
                "type": "text",
                "text": prompt.system_prompt,
                "cache_control": {"type": "ephemeral"},
            }
        ]

    @staticmethod
    def _read_api_key() -> Optional[str]:
        """Read Anthropic API key from environment.

        Returns:
            API key string or None if not set.
        """
        import os
        key = os.environ.get("ANTHROPIC_API_KEY")
        if not key:
            logger.warning("ANTHROPIC_API_KEY not set in environment")
        return key

    def _init_client(self) -> Optional[Any]:
        """Initialize the Anthropic client SDK.

        Returns:
            Anthropic client instance or None if SDK not available or key missing.
        """
        if not self._api_key:
            return None
        try:
            import anthropic  # type: ignore[import-untyped]
            return anthropic.Anthropic(api_key=self._api_key, timeout=self._timeout_secs)
        except ImportError:
            logger.warning("anthropic SDK not installed. Install: pip install anthropic")
            return None
        except Exception as exc:
            logger.error("ClaudeProvider: client init failed", extra={"error": str(exc)})
            return None

    @staticmethod
    def _mock_response() -> Dict[str, Any]:
        """Return a deterministic mock response when no client is available.

        Returns:
            Safe fallback routing response.
        """
        return {
            "action": "DELIVER_SILENTLY",
            "reason": "Claude API unavailable; safe default applied.",
            "confidence": 0.50,
            "evidence": [],
        }

    @property
    def model(self) -> str:
        """Return the Claude model identifier."""
        return self._model

    @property
    def is_available(self) -> bool:
        """Return True if the Claude client is ready for API calls."""
        return self._client is not None
